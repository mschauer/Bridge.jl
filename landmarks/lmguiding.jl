...
"""
function update_initialstate!(X,Xᵒ,W,ll,x,xᵒ,∇x, ∇xᵒ,result, resultᵒ,
                sampler, Q,mask, mask_id, δ, ρ, acc)
    n = Q.target.n
    ...
    if sampler==:mcmc
        ...
        cfgᵒ = ForwardDiff.GradientConfig(slogρ(Q, W, Xᵒ), xᵒ, ForwardDiff.Chunk{2*d*n}()) # 2*d*P.n is maximal
