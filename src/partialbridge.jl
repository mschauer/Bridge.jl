...
function partialbridgeode!(::R3, t, L, Σ, Lt, M⁺t, μt, P)
    m, d = size(L)
    Lt[end] = L
    M⁺t[end] = M⁺ = Σ   
    μt[end] = μ = 0*L[:,1]
    ...
    for i in length(t)-1:-1:1
        ...
        M⁺ = kernelr3((t, y, (L,P)) -> -L*a(t, P)*L', t[i+1], M⁺, dt, (L,P))  
...
struct PartialBridge{T,R,R2,Tv,TL,TM} <: ContinuousTimeProcess{T}
    ...
    function PartialBridge(tt_, P, Pt, L::TL, v::Tv, Σ::TM = Bridge.outer(zero(v))) where {TL, Tv, TM}
        ...
        M⁺t = zeros(TM, N)
        μt = zeros(Tv, N) 
        partialbridgeode!(R3(), tt, L, Σ, Lt, M⁺t, μt, Pt)
        new{Bridge.valtype(P),typeof(P),typeof(Pt),Tv,TL,TM}(P, Pt, tt, v, Lt, M⁺t, μt)
    end
end
...
Hi(i::Integer, x, P::PartialBridge) = P.L[i]' * inv(P.M⁺[i]) * P.L[i] 
