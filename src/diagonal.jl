...
#     expr = [:((SVector(a[:,$i]) * b[$i])) for i=1:N]
#     :( SMatrix($(expr...)) )
# end
# @generated function scalem{T, M, N}(b::SVector{M,T}, a::SMatrix{M, N, T})
#     expr = [:((SVector(a[$i,:]) * b[$i])) for i=1:M]
...
# function scalem{T, M}(a::SVector{M,T}, b::SVector{M, T})
#    a.*b
# end
